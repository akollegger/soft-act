...
const generateCode = (privateKeyPath, activationData) => {
  const license = jsYaml.safeDump(activationData);
        ...
        .then( ([err, signature]) => {
          ...
          return(
            ...
            '# NEO4J SOFTWARE FEATURE ACTIVATION CODE\n' +
    				jsYaml.safeDump(activationData));
    });
  });
};
...
const verifyCode = (publicKeyPath, signedActivationCode) => {
  return cryptoPromise.readPublicKey(publicKeyPath)
    .then( ([err, publicKey]) => {
      const fullactivationData = jsYaml.safeLoad(signedActivationCode);
      const signature = fullactivationData.signature;
      delete fullactivationData.signature;
      const partialLicense = jsYaml.safeDump(fullactivationData);
...
const register = (registrationData) => {
  return jsYaml.safeDump(registrationData);
};

const recall = (encodedRegistration) => {
  return jsYaml.safeLoad(encodedRegistration);
};
